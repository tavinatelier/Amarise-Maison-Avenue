    ...
    <Layout>
      ...
      <section className="pt-32 pb-16 md:pt-40 md:pb-24">
        <div className="container-editorial text-center">
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-caption mb-4">The World of AMARISÉ</motion.p>
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="font-serif text-4xl md:text-6xl lg:text-7xl mb-6">
            {pillarData.name}
          </motion.h1>
        ...
        <div className="container-editorial">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
            {pillarData.families.map((family, i) => (
              <motion.div key={family.slug} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}>
                <Link to={`/shop/${pillarData.slug}/${family.slug}`} className="group block">
                  <div className="editorial-card aspect-[3/4] bg-muted mb-4">
                    <img src={family.image || "/placeholder.svg"} alt={family.name} className="editorial-card-image" />
                  </div>
                  <div className="flex items-baseline justify-between">
                    <h3 className="font-serif text-lg">{family.name}</h3>
                    <span className="text-caption opacity-0 group-hover:opacity-100 transition-opacity">{family.productCount} pieces</span>
                  </div>
      ...
      {products.length > 0 && (
        <section className="section-luxury-sm border-t border-border">
          <div className="container-editorial">
            ...
            <ProductFilterEngine products={products}>
              {(filteredProducts) => (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {filteredProducts.map((product) => {
                    ...
                    return (
                      <Link key={product.id} to={`/shop/${product.pillarSlug}/${product.familySlug}/${product.slug}`} className="group block">
                        <div className={`editorial-card aspect-[3/4] bg-muted mb-3 relative ${isSold ? "opacity-60" : ""}`}>
                          <img src={product.images[0]} alt={product.title} className="editorial-card-image" />
                          {isLow && <span className="absolute top-3 left-3 text-[10px] tracking-widest uppercase px-2 py-1 bg-muted text-muted-foreground">Low Stock</span>}
